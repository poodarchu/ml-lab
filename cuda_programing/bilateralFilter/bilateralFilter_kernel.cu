 ...
 * In a computing application, this roughly implies that a thread is likely to read from an address “near” the address that nearby threads read
 */
texture<uchar4, 2, cudaReadModeNormalizedFloat> rgbaTex;   //Texture (<type>, <dim>, <readmode>) <texture_reference>
 ...
 *
 * 双边滤波器是一个非线性滤波器，它是一个 range filter 和一个 domain filter 的混合体。
 * range filter 保留清晰的边缘，而 domain filter 用于过滤噪声。
 * 一个像素点的 intensity value 被它周围的点的加权值替代。
 *
 * 其中，权值银子通过 domain filter 的 component（使用高斯分布作为空间分布）） 和 range filter 的 component（中心像素和当前邻域内像素的欧氏距离）的乘积计算得到
 * 由于这个过程是非线性的，所以采用的是逐点运算的方式。
 *
 * 纹理 Texture 自动钳点到图像边缘。1D 高斯数组映射为 1D 的纹理，而不是使用 shared memory，因为那将会导致严重的 bank conflict 问题。
 *
 * 线程是纵向执行的，因为输出是合并的（coalesced）
 *
 * 参数：
 ...
 */
 ...
 __global__ void d_bilateral_filter(uint* od, int w, int h, float e_d, int r) {
   ...
   //tex2D - performs a texture lookup in a given 2D sampler and,
   //in some cases, a shadow comparison. May also use pre computed derivatives if those are provided.
   float4 center = tex2D(rgbaTex, x, y);

   for (int i = -r; i <= r; i++) {
     for (int j = -r, j <= r; j++) {
       float4 curPix = tex2D(rgbaTex, x+j, y+i);
       factor = cGaussian[i+r] * cGaussian[j+r] * //domian factor
